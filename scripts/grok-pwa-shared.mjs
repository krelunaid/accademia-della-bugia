...
export function grokPwaHeadTags(appName = DEFAULT_APP_NAME, shortName = appName) {
  const label = String(shortName || appName).trim() || DEFAULT_APP_NAME;
  return [
    // Standalone display comes from the manifest ("display": "standalone");
    // the legacy *-web-app-capable metas it replaces are deliberately absent.
    ["manifest", '<link rel="manifest" href="/__grok/manifest.webmanifest">'],
    ["apple-touch-icon", '<link rel="apple-touch-icon" sizes="180x180" href="/icon-180.png">'],
    [
      "apple-mobile-web-app-title",
      `<meta name="apple-mobile-web-app-title" content="${escapeHtml(label)}">`,
    ],
    [
...
export function injectGrokPwaHead(html, ctx = {}) {
  ...
  const shortName = String(site.short_name ?? "").trim() || appName;
